...
def train():
    ...
    for epoch_id in range(epochs):
        ...
        for batch_id, data in enumerate(train_dataloader):
            ...
            if len(inputs) < len(ctx):
                div = (len(ctx) + len(inputs) - 1) // len(inputs)
                inputs = mx.nd.repeat(inputs, repeats=div)
                token_types = mx.nd.repeat(token_types, repeats=div)
                valid_length = mx.nd.repeat(valid_length, repeats=div)
                start_label = mx.nd.repeat(start_label, repeats=div)
                end_label = mx.nd.repeat(end_label, repeats=div)
                continue
